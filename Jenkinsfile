            ...
            script {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        ...
                        sh "cat cfg/dbt.yaml"
                        sh "sed -i 's+948065143262.dkr.ecr.us-east-1.amazonaws.com/patrick-cloud-dev-dbt-docs.*+948065143262.dkr.ecr.us-east-1.amazonaws.com/patrick-cloud-dev-dbt-docs:+g' cfg/dbt.yaml"
                        sh "cat cfg/dbt.yaml"
                        sh "git add ."
